...
CAPTCHA_PHRASES = [
    "captcha", "капча", "robot", "автоматические запросы",
    "подозрительная активность", "подтвердите", "слишком много запросов",
    "блокировка ip", "firewall",
]
...
def parse() -> list:
    """Цепочка: RSS (curl_cffi) → Firefox Playwright → Stealth Chromium."""
    # 1. RSS — без Playwright, без fingerprint
    try:
        ...
        if results:
            ...
            return results
        logging.info("Avito RSS: 0 результатов — fallback на Firefox Playwright")
    except Exception as e:
    ...
    if results:
        return results
    logging.info("Avito Firefox: 0 результатов — fallback на Stealth Chromium")

    # 3. Stealth Chromium (последний резерв)
    try:
        from parsers.avito_playwright import parse as stealth_parse
        results = stealth_parse()
        logging.info(f"Avito Stealth Chromium: {len(results)} объявлений")
        return results
    except Exception as e:
        logging.error(f"Avito Stealth fallback: {e}")
    return []
